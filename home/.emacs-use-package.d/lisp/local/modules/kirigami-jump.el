...
(defun kirigami-jump--outline-after-jump-when-not-minibuffer ()
  ...
  (unless (minibufferp)
    ...
    (run-with-idle-timer 0 nil #'kirigami-jump--open-fold)))
...
(defun kirigami-jump--ediff-prepare-buffer ()
  ...
  (if enable
      (progn
        ...
        (with-eval-after-load 'evil
          ...
          ;; Search with n
          ;; NOTE causes problems. cursor moves on its own somes, not where
          ;; the searched text is
          ;; TODO too slow. implement restoring window start when it opens for real
          ;; (advice-add 'evil-ex-search :around #'kirigami-jump--around-outline-show-entry)

          ;; Search with /
          ;; NOTE causes problems. cursor moves on its own somes, not where
          ;; the searched text is
          ;; TODO too slow. implement restoring window start when it opens for real
          ;; (advice-add 'evil-ex-search-forward :around #'kirigami-jump--around-outline-show-entry)
